...
def detection_target_layer(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    if torch.nonzero(gt_class_ids < 0).size() and torch.nonzero(gt_class_ids < 0).size() != torch.Size([0]):
    ...
    # Positive ROIs
    if torch.nonzero(positive_roi_bool).size() and torch.nonzero(positive_roi_bool).size() != torch.Size([0]):
...
class MaskRCNN(nn.Module):
    ...
    def valid_epoch(self, datagenerator, steps):
        ...
        for inputs in datagenerator:
            ...
            if not target_class_ids.size() or target_class_ids.size()==torch.Size([0]):
                continue
